...
{
    public class Test70_LDN_NormalItem_UnderFreeShippingLimit_UKExpedited : BaseBrowserFixture
    {
        ...
        [Test]
        public void TestMyUseCaseSeventy()
        {
            ...
            Debug.WriteLine("Information on Product Page: Product Price is:" + _productPrice);
            ...
            shippingBillingPage.TypeFirstName(Configuration.GetNAFirstName);
            shippingBillingPage.TypeLastName(LastName);
            shippingBillingPage.TypeAddressLine1(Configuration.GetNAAddressLine1);
            shippingBillingPage.TypeCity(Configuration.GetNACity);
            shippingBillingPage.SelectCountry(Configuration.GetNACountry);
            shippingBillingPage.SelectState(Configuration.GetNAState);
            shippingBillingPage.TypePostalCode(Configuration.GetNAPostalCode);
            shippingBillingPage.TypePhone(Configuration.GetNAPhone);
            ...
            shippingBillingPage.WaitForElement(By.Id("shipping-option-United Kingdom Expedited"), 10);
            Assert.False(shippingBillingPage.IsGroundShippingNotPresent(), "Ground Shipping option is present.");
            Assert.True(shippingBillingPage.IsUKExpeditedPresent(), "UK Expedited option is not present.");
            _taxes = shippingBillingPage.GetTaxes();
            ...
            Assert.True(_shippingInfo.ToLower().Contains(Configuration.GetNAFirstName.ToLower()),
            ...
            Assert.True(_shippingInfo.ToLower().Contains(Configuration.GetNAAddressLine1.ToLower()),
                "Shipping To - Address line1 is not present on Order Summary page.");
            Assert.True(_shippingInfo.Contains(Configuration.GetNACity),
                "Shipping To - City is not present on Order Summary page.");
            Assert.True(_shippingInfo.Contains(Configuration.GetUKCountryCode),
                "Shipping To - Country is not present on Order Summary page.");
            Assert.True(_shippingInfo.Contains(Configuration.GetNAState),
                "Shipping To - State code is not present on Order Summary page.");
            Assert.True(_shippingInfo.Contains(Configuration.GetNAPostalCode),
                "Shipping To - Postal code is not present on Order Summary page.");
            Assert.True(orderSummaryPage.GetShippingMethod().Contains("United Kingdom Expedited"),
                "Shipping method is not present on Order Summary page.");

            _billingInfo = orderSummaryPage.GetBillingInformation();
            Assert.True(_billingInfo.ToLower().Contains(Configuration.GetNAFirstName.ToLower()),
            ...
            Assert.True(_billingInfo.ToLower().Contains(Configuration.GetNAAddressLine1.ToLower()),
                "Billing To - Address line1 is not present on Order Summary page.");
            Assert.True(_billingInfo.Contains(Configuration.GetNACity),
                "Billing To - City is not present on Order Summary page.");
            Assert.True(_billingInfo.Contains(Configuration.GetUKCountryCode),
                "Billing To - Country is not present on Order Summary page.");
            Assert.True(_billingInfo.Contains(Configuration.GetNAState),
                "Billing To - State code is not present on Order Summary page.");
            Assert.True(_billingInfo.Contains(Configuration.GetNAPostalCode),
